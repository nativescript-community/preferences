...
export class Preferences extends Common {
    ...
    private mNativeObservers: { [k: string]: NSObject } = {};
    constructor() {
        super();
    ...
    onListenerAdded(eventName: string, count: number): void {
        if (eventName.startsWith('key:') && count === 1) {
            ...
            if (!this.mNativeObservers[key]) {
                this.mNativeObservers[key] = ObserverClass.alloc().init();
                this.mNativeObservers[key]['owner'] = this;
            }
            this.userDefaults.addObserverForKeyPathOptionsContext(this.mNativeObservers[key], key, NSKeyValueObservingOptions.New, null);
        }
    }
    onListenerRemoved(eventName: string, count: number): void {
        if (eventName.startsWith('key:') && count === 0) {
            const key = eventName.replace('key:', '');
            if (this.mNativeObservers[key]) {
                this.userDefaults.removeObserverForKeyPath(this.mNativeObservers[key], key);
                this.mNativeObservers[key] = null;
                delete this.mNativeObservers[key];
            }
        }
    }
